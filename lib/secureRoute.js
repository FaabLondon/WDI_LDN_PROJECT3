...
//Secureroute checks for authorisation in the header, it gets back the token from the header.
...
function secureRoute(req, res, next){
  ...
  //3) get user from DB and update req.currentUser
    ...
    .then(user => {
      ...
      req.currentUser = user; //to access it later on in the controller - can be accessed on any route when you have secureRoute on the router (in the router)
